...
This module implements the core language model (Lm) for Moshi, which is a
multimodal transformer that jointly models text and audio tokens using a
...
1. Main Transformer: A large transformer that processes the combined text
   and audio token embeddings. It produces contextualized representations
   that are used for text prediction.

2. DepFormer (Dependent Transformer): A smaller transformer that generates
   audio tokens conditioned on the main transformer's output. It processes
   one codebook at a time in an autoregressive manner.

The "delayed streams" architecture means that audio tokens at different
codebook levels have different delays relative to the text stream. This
allows the model to generate audio that is temporally aligned with text
while maintaining causal dependencies.

Key Classes:
-----------
- LmConfig: Configuration dataclass for the language model
- DepFormerConfig: Configuration for the dependent transformer
- Lm: The main language model class
- DepFormer: The dependent transformer for audio generation
- ScaledEmbedding: Embedding layer with optional low-rank factorization

Configuration Functions:
-----------------------
- config_v0_1(): Original Moshi configuration (7B parameters)
- config1b_202412(): 1B parameter model
- config_helium_1_preview_2b(): Text-only Helium model (2B parameters)
"""
...
class DepFormerConfig:
    """
    Configuration for the Dependent Transformer (DepFormer).
    
    The DepFormer is responsible for generating audio tokens conditioned
    on the main transformer's output. It processes codebooks sequentially,
    with each slice generating one codebook level.
    
    Attributes:
        transformer: Configuration for the transformer layers
        num_slices: Number of audio codebooks to generate (typically 8)
...
class LmConfig:
    """
    Configuration for the main Language Model.
    
    This dataclass holds all configuration parameters for the Lm model,
    including transformer architecture, vocabulary sizes, audio settings,
    and conditioning options.
    
    Attributes:
        transformer: Configuration for the main transformer
        depformer: Configuration for the dependent transformer
        text_in_vocab_size: Input text vocabulary size (includes special tokens)
        text_out_vocab_size: Output text vocabulary size
        ...
        conditioners: Dictionary of conditioner configurations
        demux_second_stream: Whether to demultiplex a second input stream
        extra_heads_num_heads: Number of extra prediction heads
    ...
    def other_codebooks(self):
        """Number of audio codebooks from input (not generated)."""
    ...
    def from_config_dict(cls, data: dict) -> "LmConfig":
        ...
        This method handles the conversion from the JSON configuration format
        used in model checkpoints to the structured LmConfig dataclass.
        
        Args:
            data: Dictionary containing model configuration
        
        Returns:
            LmConfig instance with all parameters set
        """
        transformer = TransformerConfig(
            d_model=data["dim"],
...
class ScaledEmbedding(nn.Embedding):
    ...
    This embedding layer extends the standard nn.Embedding with several features:
    
    1. Zero Index: A special index value that produces zero embeddings, useful
       for masking or indicating "no input" positions.
    
    2. Low-Rank Factorization: Optionally factorizes the embedding matrix into
       two smaller matrices, reducing memory for large vocabularies.
    
    3. Stream Demultiplexing: For models with two input streams multiplexed into
       a single token, this layer can demultiplex and embed both streams.
    
    The demultiplexing works by encoding two tokens as: combined = tok2 * card + tok1
    where card is the vocabulary size. The layer then extracts both tokens and
    applies separate linear transformations.
    
    Args:
        num_embeddings: Size of the vocabulary
        embedding_dim: Dimension of the embedding vectors
        zero_idx: Special index that produces zero embeddings (must be negative)
...
class DepFormerSlice(nn.Module):
    """
    A single slice of the DepFormer that generates one audio codebook.
    
    Each slice takes the main transformer output and the previous token
    (either text for the first slice, or the previous audio codebook),
    and predicts the next audio token for its assigned codebook level.
    
    The slice consists of:
    1. An embedding layer for the input token
    2. A linear projection from the main transformer dimension
    3. A small transformer for processing
    4. An output linear layer for token prediction
    
    Args:
        in_vocab_size: Input vocabulary size (text or audio)
        out_vocab_size: Output vocabulary size (audio codebook size - 1)
        main_transformer_dim: Dimension of the main transformer output
        demux_second_stream: Whether to demultiplex two streams
...
class DepFormer(nn.Module):
    """
    Dependent Transformer for autoregressive audio token generation.
    
    The DepFormer generates audio tokens conditioned on the main transformer's
    output. It consists of multiple "slices", each responsible for generating
    one level of the audio codebook hierarchy.
    
    The generation is autoregressive across codebooks:
    1. First slice takes text token and main transformer output
    2. Each subsequent slice takes the previous audio token
    3. All slices share the same transformer weights (with optional scheduling)
    
    This design allows efficient generation of multi-codebook audio while
    maintaining proper dependencies between codebook levels.
    ...
    def sample(
        ...
        """
        Sample audio tokens from all depformer slices.
        
        Generates audio tokens autoregressively across codebook levels,
        with each slice conditioned on the previous slice's output.
        
        Args:
            main_transformer_out: Output from the main transformer [B, 1, D]
...
class Lm(nn.Module):
    """
    Main Language Model for Moshi.
    
    This is the core model class that combines a main transformer with a
    dependent transformer (DepFormer) to jointly model text and audio.
    
    Architecture:
    1. Embeddings: Separate embeddings for text and each audio codebook
    2. Main Transformer: Processes combined embeddings to produce contextualized
       representations for text prediction
    3. DepFormer: Generates audio tokens conditioned on transformer output
    4. Conditioning: Optional conditioning via lookup tables or tensors
    
    The model supports:
    - Autoregressive text and audio generation
    - Classifier-free guidance (CFG) for improved quality
    - Quantization for reduced memory usage
    - Multiple conditioning modalities (quality, speaker, etc.)
